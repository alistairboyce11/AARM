...
# Assumed sac file format YYYMMDDHHMMSS_NETWORK_STATION.??.?HZ
...
# rm *out *stk *stk2 *corr *corr2 *new gmt* *.m *.s *.cut *.cut2
...
sactosac -f *.?HZ.stk.s
saclst user0 user5 f *.?HZ.stk.s > rms_pre_arrival_signal.out # filename, rms of data in noise window, rms of data in signal window
sactosac -m *.?HZ.stk.s
awk '{print $1, ($3/$2)}' rms_pre_arrival_signal.out | sed 's/\HZ.stk.s /HZ /g' > SNR.txt
...
sactosac -f *.corr
saclst depmax t9 f *.corr | grep HZ > correlation.out # filename, correlation co-efficient, XC location (correction)
sactosac -m *.corr

# Calculate mean XC co-efficient between traces and stack to compare with 2nd stack.
grep HZ correlation.out | awk '{print $2}' > XC_coefficients.out # correlation co-efficients
...
then
	...
	echo "read *HZ; write append .new" > mk_stk2.m
...
# C1.9
	...
	then
		...
		echo "m correct_T0.m" > mk_stk2.m
		echo "read *HZ.new" >> mk_stk2.m
		echo "cuterr fillz" >> mk_stk2.m
		echo "cut t0 -30 30" >> mk_stk2.m
		echo "read" >> mk_stk2.m
		echo "synchronize" >> mk_stk2.m
		echo "decimate 2" >> mk_stk2.m
		echo "interpolate delta 0.025" >> mk_stk2.m
		# echo "int" >> mk_stk2.m # optional
		echo "taper width 0.3" >> mk_stk2.m
		echo "chnhdr b -30" >> mk_stk2.m
...
grep HZ correlation2.out | sort -k3 -n > correlation_sort2.txt # Sorted by third column.
...
then
	STAT=`echo $line | sed 's/\_/ /g' | sed 's/\./ /g' | awk '{print $3}'`
	FILE=`echo $line | awk '{print $1}'`
	ORIG_FILE=`echo $FILE | sed 's/\.stk2.cut2.corr2/ /g' | sed 's/\.new/ /g'` # will remove all file suffixes from either NOISE or XC regime.
...
# C1.13
list=`sort -k1 correlation_sort2.txt | sed 's/\_/ /g' | sed 's/\./ /g' | awk '{print $3}'`
# echo $list
...
sactosac -f *.?HZ.new
saclst o t0 t1 f *.?HZ.new > final_headers.out  # filename, omarker, Rel-Arr alignment, Predicted arrival
sactosac -m *.?HZ.new
